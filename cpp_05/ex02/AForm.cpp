...
AForm::AForm( void ) : _gradeSign(0), _gradeExec(0), _signed(false), _name( "default_Name" ) {
...
AForm::AForm( AForm const & src ) : _gradeSign(src._gradeSign), _gradeExec(src._gradeExec), _name( src._name ) {
...
void AForm::setTarget( std::string target ) { const_cast<std::string&>( _name ) = target; }
...
void        AForm::beSigned( Bureaucrat & ref ) {
    ...
    if ( ref.getGrade() <= _gradeSign ) {
        ...
        throw gradeExcept( RED "AForm::GradeTooLowException" RESET );
    }
}
...
void        AForm::checkAccess( Bureaucrat const & executor ) const {
    ...
    if ( getSigned() == false ) { throw gradeExcept( "ShrubberyCreationForm::UnsignedFormException" ); }
    if ( executor.getGrade() > getGradeExec() ) { throw gradeExcept( "ShrubberyCreationForm::GradeExecToLowException" ); }
